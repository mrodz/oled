...
namespace ui
{
	...
	{
		if (event->code == LV_EVENT_DELETE && event->target == login_mbox)
		{
			/* Delete the parent modal background */
			lv_obj_del_async(lv_obj_get_parent(login_mbox));
			login_mbox = nullptr; /* happens before object is actually deleted! */
		}
	...
	static char profile_edit_title_buf[PROFILE_EDIT_TITLE_BUF_LEN];
	...
	static void profile_edit_click_cb(lv_event_t *event)
	{
		if (event->code == LV_EVENT_CLICKED)
		{
			...
			memset(profile_edit_title_buf, 0, sizeof(profile_edit_title_buf));
			snprintf(profile_edit_title_buf, sizeof(profile_edit_title_buf), "Edit %s", src);
			profile_edit_title_buf[sizeof(profile_edit_title_buf) - 1] = '\0';

			Serial.print("@@@ ");
			Serial.println(profile_edit_title_buf);

			login_mbox = lv_msgbox_create(obj, profile_edit_title_buf, "", nullptr, true);

			lv_obj_set_align(login_mbox, LV_ALIGN_CENTER);
			lv_obj_add_event_cb(
				login_mbox, profile_edit_modal_cb,
				LV_EVENT_ALL, nullptr);

			lv_obj_t *b = lv_btn_create(login_mbox);
	...
	static void add_user_modal_cb(lv_event_t *event)
	{
	...
	static void add_user_cb(lv_event_t *event)
	{
		if (event->code == LV_EVENT_CLICKED)
		{
			/* Create a base object for the modal background */
